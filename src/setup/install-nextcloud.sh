...
# install-nextcloud.sh - Installation script for Nextcloud
# This script handles ONLY the installation of Nextcloud
# Configuration is handled by configure-nextcloud.sh
...
# Component details
COMPONENT="nextcloud"
NEXTCLOUD_VERSION="27.1.3"  # Latest stable version as of October 2024
NEXTCLOUD_URL="https://download.nextcloud.com/server/releases/nextcloud-${NEXTCLOUD_VERSION}.tar.bz2"
NEXTCLOUD_CHECKSUM_URL="${NEXTCLOUD_URL}.sha256"
NEXTCLOUD_CHECKSUM_SIG_URL="${NEXTCLOUD_URL}.sha512"
...
main() {
    ...
    if [ -f "/var/www/nextcloud/config/config.php" ]; then
        print_error "Nextcloud is already installed. Use the update script to upgrade."
        exit 1
    fi
    ...
    echo -e "\nRun '${YELLOW}./nextcloud-setup configure nextcloud${NC}' to configure Nextcloud\n"
}
...
install_nextcloud() {
    ...
    local web_root="/var/www/nextcloud"
    mkdir -p "$web_root"
    
    # Download Nextcloud
    print_status "Downloading Nextcloud ${NEXTCLOUD_VERSION}..."
    local temp_dir
    temp_dir=$(mktemp -d)
    ...
    if ! wget -q "$NEXTCLOUD_URL" -O nextcloud.tar.bz2 || \
       ! wget -q "$NEXTCLOUD_CHECKSUM_URL" -O nextcloud.tar.bz2.sha256; then
        print_error "Failed to download Nextcloud or checksum file"
        exit 1
    fi
    
    # Verify checksum
    if ! sha256sum -c nextcloud.tar.bz2.sha256 --status; then
        print_error "Nextcloud checksum verification failed"
        exit 1
    fi
    
    # Extract Nextcloud
    print_status "Extracting Nextcloud..."
    tar -xjf nextcloud.tar.bz2 --strip-components=1 -C "$web_root"
    
    # Set permissions
    print_status "Setting permissions..."
    local apache_user="www-data"
    local apache_group="www-data"
    
    # Create data directory
    local data_dir="${NEXTCLOUD_DATA_DIR:-/var/nextcloud-data}"
    mkdir -p "$data_dir"
    
    # Set ownership and permissions
    chown -R "$apache_user:$apache_group" "$web_root" "$data_dir"
    find "$web_root" -type d -exec chmod 750 {} \;
    find "$web_root" -type f -exec chmod 640 {} \;
    
    # Make sure the config directory is writable during installation
    chmod 770 "$web_root/config"
...
EOF
    ...
    print_status "Nextcloud files installed to $web_root"
}

# Save installed version to .env
save_version() {
    set_env "NEXTCLOUD_VERSION" "$NEXTCLOUD_VERSION"
    
    # Mark as installed
    touch "/etc/nextcloud/.nextcloud_installed"
    chmod 600 "/etc/nextcloud/.nextcloud_installed"
}
...
# 1. Install php-fpm
log_info "Installing PHP 8.4 FPM..."
...
log_section "NEXTCLOUD INSTALLATION COMPLETE"
echo ""
echo "╔════════════════════════════════════════════════════════════════════════════╗"
