...
NEXTCLOUD_PATH="${NEXTCLOUD_ROOT:-/var/www/nextcloud}"
WEB_SERVER_USER="${WEB_SERVER_USER:-www-data}"
LOG_DIR="${LOG_DIR:-/var/log/nextcloud}"

# Ensure logs directory exists
mkdir -p "$LOG_DIR"
chmod 755 "$LOG_DIR"
chown ${WEB_SERVER_USER}:${WEB_SERVER_USER} "$LOG_DIR"

# Set up log file
LOG_FILE="${LOG_DIR}/monitor-$(date +%Y%m%d).log"
STATUS_FILE="${LOG_DIR}/status.json"
# Require PHP 8.4
PHP_PATH="${PHP_BIN:-$(which php8.4)}"
if [ -z "$PHP_PATH" ]; then
...
# Main function
main() {
    log_section "Starting Nextcloud Monitoring"
    
    # Create status directory if it doesn't exist
    local status_dir=$(dirname "$STATUS_FILE")
    mkdir -p "$status_dir"
    chmod 750 "$status_dir"
    chown ${WEB_SERVER_USER}:${WEB_SERVER_USER} "$status_dir"
    
    log_info "Monitoring Nextcloud at: $NEXTCLOUD_PATH"
    log_info "Using PHP: $PHP_PATH"
    
    # Generate status
    local status
    if ! status=$(generate_status); then
        log_error "Failed to generate status"
        return 1
    fi
    ...
    chown ${WEB_SERVER_USER}:${WEB_SERVER_USER} "$STATUS_FILE"
    
    # Check for issues
    if ! check_for_issues "$status"; then
